...
const actions = {
  ...
  getWinesForFood({sessionId, context, flavor}) {
    ...
    console.log('------> getWinesForFood:  ',{sessionId, context, flavor})

    const recipientId = sessions[sessionId].fbid;

    context.flavor = flavor;

    return sendGenericMessage(recipientId)
        .then(() => {
          console.log('worked?');
        })
        .catch((err) => {
    ...
    data.entry.forEach(entry => {
      entry.messaging.forEach(event => {
        if (event.message) {
          ...
          facebook.get.getUserProfile(sender).then((user) => {
            console.log('USERS NAME: ',user.first_name);
            sessions[sessionId].context.first_name = user.first_name;
            sessions[sessionId].context.last_name = user.last_name;
          });
          ...
          if (attachments) {
            ...
            facebook.send.text(sender, 'Sorry I can only process text messages for now.')
            .catch(console.error);
          } else if (text) {
            ...
            console.log('**************************************************');
            console.log('Current context before we send it to wit.ai');
            console.log(sessionId, sessions[sessionId].context);
            console.log('**************************************************');
            wit.runActions(
