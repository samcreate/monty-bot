...
class BootBot extends EventEmitter {
  ...
  runAIRequest(event = {}, session, context, typing_time) {
    ...
    let request = this.apiai.textRequest(event.message.text, {
      ...
      console.log('   @--- lapiai response')
      let res_msgs = response.result.fulfillment.messages;

      let qr = {};
      res_msgs.forEach((msg, i) => {
        if (msg.replies) {
          qr.quickReplies = res_msgs.splice(i, 1)[0].replies
          res_msgs.forEach((msg2, z) => {
            if (msg2.speech == response.result.fulfillment.speech) {
              qr.text = res_msgs.splice(z, 1)[0].speech
            }
          });
        }
      });
      if('text' in qr) res_msgs.unshift(qr);
  ...
  _handle_messages_fromapai(messages, session, resolve, reject){
    ...
    if(messages.length >= 1){
      ...
      let promise;
      if(tmp_msg.text){
        console.log('   @--- sending quickreply message ', tmp_msg.text)
        promise = this.say(session.get('UserUid'), tmp_msg, {
          typing: true
        });
      }else if(tmp_msg.speech){
        console.log('   @--- sending a reqgular message ', tmp_msg.speech)
        promise = this.say(session.get('UserUid'), tmp_msg.speech, {
          typing: true
        });
      } else if(tmp_msg.imageUrl){
