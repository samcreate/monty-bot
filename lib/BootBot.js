...
class BootBot extends EventEmitter {
  ...
  runAIRequest(event = {}, session, context, typing_time) {
    ...
    let request = this.apiai.textRequest(event.message.text, {
      ...
      this._checkForMedia(response.result.fulfillment.speech)
      .then((media_msgs)=>{
        return new Promise((resolve, reject) => {
          if(media_msgs){
            console.log('   @--- message had media, lets procces it and send.')
            this._handle_send_media_msgs(media_msgs, session, resolve)
          }else {
            console.log('   @--- no media, let\'s coninute.')
            resolve();
          }
        })
      })
      .then((res)=>{
        //if the response from media message handleing is that it's already sent do nothing or let's go ahead and proccess a
        //'normal' message

        if(res !== 'finished'){
          if(!response){
            console.log('   @--- api.ai send nothing to repond to.', response)
            return default_response(session.get('UserUid'), 'no text');
          }else if(already_sent != true){
            console.log('   @--- sending a simple message')
            this._checkForQuickReplies(response.result.fulfillment.speech).then(({text, quickreplies}) => {
              //respond to user
              if(text && quickreplies){
                console.log('   @--- sending reponse with quickReplies')
                already_sent = true
                return this.say(session.get('UserUid'), {
                  text: text,
                ...
                return this.say(session.get('UserUid'), response.result.fulfillment.speech, {
                  typing: (typing_time || true )
                });
              }
            });
          } //already_sent != true
      ...
      .catch((text) => {
        default_response(session.get('UserUid'), 'An error occured');
      });
  ...
  _handle_send_media_msgs(msgs, session, resolve, msg_sent = false){
    ...
    if(msgs.length >= 1){
      ...
      if(tmp_msg.indexOf('@') != -1){
        ...
        this._checkForQuickReplies(tmp_msg).then(({text, quickreplies}) => {
          //respond to user
          if(text && quickreplies){
            console.log('   @--- sending reponse with quickReplies')
             this.say(session.get('UserUid'), {
