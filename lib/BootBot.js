...
class BootBot extends EventEmitter {
  ...
  runWitActions(event, sessionId){
    console.log('runWitActions');
    this.wit.runActions(
  ...
  _handleMessageEvent(event, sessionId) {
    ...
    if(this._hearMap.length === 0){
      this.runWitActions(event, sessionId);
    }
  }
  ...
  _initWebhook() {
    this.app.get('/sessions', (req, res) => {
      res.json(Sessions.instance.store);
    });
      ...
      data.entry.forEach((entry) => {
        ...
        entry.messaging.forEach((event) => {
          ...
          if (event.optin) {
            ...
            this._handleMessageEvent(event, sessionId);
